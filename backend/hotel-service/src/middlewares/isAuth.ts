...
export const isHotelOwner = (req: AuthRequest, res: Response, next: NextFunction) => {
  // Hotel owner check: user must have role 'hotel_owner' or be admin
  // We'll add a simple role check – you can expand later
  if (req.userRole !== 'hotel_owner' && req.userRole !== 'admin') {
    return res.status(403).json({ success: false, error: 'Hotel owner access required' });
  }
  next();
};